...
strategy = random.randint(4,4)       ##//selected strategy // estratégia selecionada
...
def sensors(front_left, front_right, back_left, back_right):
    ...
    elif front_right > 0.1 and back_right > 0.1:
    ...
    elif front_left > 0.1 and back_left > 0.1:
        ##//turn to the left // gira para a esquerda
        left_speed = speed
        right_speed = -speed
        flag = 0
    ...
    elif front_right>0 and front_left>0:
        left_speed = speed
        right_speed = -speed
...
def control(front_right, front_left, back_right, back_left, distance_right, distance_left):
    ...
    if strategy == 1:
        if inicio == 'start':
            left_speed, right_speed, flag = start(strategy)
            if count > 8:
                inicio = 'neutral'
        else:
    ...
    return {
        ...
        'log': [ 
          ...
          #  { 'name': 'erro',  'value':   erro,    'min': -500, 'max': 500 },
           { 'name': 'Dist Right', 'value':distance_right,   'min': 0, 'max': 300 },
           { 'name': 'Dist Left', 'value':distance_left, 'min': 0, 'max': 300 }
